...
public class FailureControllerTest {
    ...
    public void testIndex() {
    	running(fakeApplication(inMemoryDatabase()), new Runnable() {
    	       public void run() {
			    	Result result = FailureController.index();
    ...
    public void testShow(){
    	...
    	running(fakeApplication(inMemoryDatabase()), new Runnable() {
	 	       public void run() {
			        ...
			        Result result = FailureController.show(timestamp);
			        assertThat(contentType(result)).isEqualTo("application/json");
			        assertThat(charset(result)).isEqualTo("utf-8");
			        assertThat(contentAsString(result)).contains("message");
			        assertThat(contentAsString(result)).contains("data");
    ...
    public void testDelete(){
    	running(fakeApplication(inMemoryDatabase()), new Runnable() {
	 	       public void run() {
	 	    	  ...
	 	    	  Result result = FailureController.delete(LongFakeID);
    ...
    public void testUpdate(){
    	running(fakeApplication(inMemoryDatabase()), new Runnable() {
	 	       public void run() {
	 	    	  String FakeID = "12564789331";
	 	    	  Long LongFakeID = Long.parseUnsignedLong(FakeID);
	 	    	  Result result = FailureController.update(LongFakeID);
	 	    	  assertThat(status(result)).isEqualTo(403);
	 	       }
    ...
    public void testCreateArray(){
    	running(fakeApplication(inMemoryDatabase()), new Runnable() {
	 	       public void run() {
			        ...
			        FakeRequest request = new FakeRequest(POST, "/failure/create").withJsonBody(json);
			        Result result = callAction(controllers.routes.ref.FailureController.create(), request);
